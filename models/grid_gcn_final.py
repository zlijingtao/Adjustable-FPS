...
class CAS(nn.Module):
    ...
    def forward(self, pos, index_voxels, neighbour_voxel_list):
        ...
        for batch in range(B):
            ...
            len_key = len(dict_keys) # This should be equal to number of nonempty voxels.
            
            
            if self.npoints <= len_key:
                # First, we random sample M points
                selected_keys = random.sample(dict_keys,self.npoints)
                unselected_keys = dict_keys - selected_keys
                H_add = 0
                H_rmv = 0
                i = 0

                # For each unselected keys, we challenge a random incumbent
                for cha_key in unselected_keys:
                    inc_key = tuple(random.sample(selected_keys, 1)[0])
                    
                    # Query neighbors of V_i to calculate H_rmv
                    for neighbour_voxel in neighbour_voxel_list[batch][inc_key[0]][inc_key[1]][inc_key[2]]:
                        voxel_tuple = tuple(neighbour_voxel.tolist())
                        val = voxels_per_batch.get(voxel_tuple)
                        if val is not None:
                            if len(val) > 1:
                                H_rmv += 1
                    
                    # Query neighbors of V_c to calculate H_add
                    for neighbour_voxel in neighbour_voxel_list[batch][cha_key[0]][cha_key[1]][cha_key[2]]:
                        voxel_tuple = tuple(neighbour_voxel.tolist())
                        val = voxels_per_batch.get(voxel_tuple)
                        if val is not None:
                            H_add += 1
                    ...
                    if H_add > H_rmv:
                        # print("Add new")
                        selected_keys.remove(inc_key)
                        selected_keys.append(cha_key)
        ...
        return centroids # centroid_index is not used
